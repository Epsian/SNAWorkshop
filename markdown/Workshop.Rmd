...
Many of the analyses discussed above cannot be run on a netowkr with multiple components. For example, if we tried to get the geodesic distances of the nodes in this netowrk, it would return only NAs, as some nodes cannot reach other nodes in the network, so their distance is unobtainable. The quickest way to deal with this situation is to reduce the network to only the largest component. In most networks, this will remove only a small amount of data, but it could be the case that you have a network that is split more evenlt; always look at your network before making such decisions. 

Taking the network from above, and extracting the largest component looks like this:
```{r largest component, message = FALSE}
capture.output({
com%v%"lc" = component.largest(com)
com = delete.vertices(com, which(com%v%"lc" == FALSE))
}, file = tempfile()) #These are just to silence some code output, remove them if you would like!
plot(com, main = "Largest Component Only")
```
